const db = require('../db')
const { RollerCoaster, ThemePark } = require('../models')

db.on('error', console.error.bind(console, 'MongoDB connection error:'))

const main = async () => {
    const magicKingdom = await ThemePark.find({ name: 'Magic Kingdom' })
    const epcot = await ThemePark.find({ name: 'Epcot' })
    const hollywoodStudios = await ThemePark.find({ name: 'Hollywood Studios' })
    const animalKingdom = await ThemePark.find({ name: 'Animal Kingdom' })
    ...
    console.log('Created RollerCoasters')
}
